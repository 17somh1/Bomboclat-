...
class CensysConnector:
    ...
    def _process_observable(self, observable):
        ...
        # Required for enrichment tracking
        work_id = self.helper.api.work.initiate_work(
            connector_id=self.helper.connect_id,
            friendly_name=f"Enriching {obs_value}"
        )
        ...
        if markdown:
            self.helper.api.note.create(
                abstract=f"Censys enrichment for {obs_value}",
                content=markdown,
                created_by=self.helper.connect_id,
                object_refs=[observable["id"]],
                note_types=["analysis"]
            )
            self.helper.log_info("[Censys] Note created and linked to observable.")
        else:
